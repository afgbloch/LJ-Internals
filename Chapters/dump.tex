...
\begin{itemize}
  ...
  \item 6th/7th column: IR operands
    ...
    \end{itemize}
    To see possible argument values for some specific instruction see from Table
    \ref{} to \ref{}.
\end{itemize}
...
This section briefly present part of the code of luaJIT involved in the process of
emitting the dump. The dump module (dump.lua) is responsible of setting up the
jit as to emit traces and also of printing the results in a human understandable
format. Depending of the dump it has to print (with respect to the provided
parameters, see Section \ref{Sec:dump-usage})
